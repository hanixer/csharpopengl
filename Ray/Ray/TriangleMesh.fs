...
let private loadResultToMesh (lr : LoadResult) =
    ...
    if lr.Groups.Count <> 1 then failwith "expected 1 group"
    let g = lr.Groups.[0]
    for face in g.Faces do
        if face.Count < 3 then failwith "expected 3 and more face points"
...
type EmptyMaterial() =
    ...
    interface IMaterialStreamProvider with
        member __.Open _ = stream

let loadFromFile filename =
    use fs = new FileStream(filename, FileMode.Open)
    let loader = ObjLoaderFactory().Create(EmptyMaterial())
    let loadResult = loader.Load(fs)
    loadResultToMesh loadResult

let meshToList (data : Data) =
    Seq.map (fun (face : Face) -> data.Vertex(face.[0]), data.Vertex(face.[1]), data.Vertex(face.[2]))
        data.Faces
